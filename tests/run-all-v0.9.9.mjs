...
need("version.file",read("VERSION").trim()==="0.9.9",`VERSION = ${read("VERSION").trim()||"absent"} (attendu 0.9.9)`);
need("version.config",contains("config.js",'APP_VERSION: "0.9.9"'),"config.js annonce 0.9.9");
need("version.config-example",contains("config.example.js",'APP_VERSION: "0.9.9"'),"config.example.js annonce 0.9.9");
need("version.cache",contains("sw.js","nid-champions-v0.9.9"),"Cache Service Worker V0.9.9");
need("version.assets",contains("assets/assets-manifest.json",'"version": "0.9.9"'),"Manifest des assets V0.9.9");
...
need("front.final.links",index.includes('href="tests/test-center-v0.9.9.html"')&&index.includes("1 880 contrôles"),"Admin pointe vers le Centre de tests V0.9.9");
...
const matrix=read("docs/TEST_MATRIX_V0.9.9.md");
const ids=[...matrix.matchAll(/\*\*(T\d{4})\*\*/g)].map(m=>m[1]);
need("tests.matrix-count",ids.length===1880,`Matrice: ${ids.length} contrôles (attendu 1880)`);
need("tests.matrix-unique",new Set(ids).size===ids.length,"IDs de la matrice uniques");
need("tests.matrix-last",ids.at(-1)==="T1880",`Dernier contrôle: ${ids.at(-1)||"absent"} (attendu T1880)`);
const center=read("tests/test-center-v0.9.9.html");
need("tests.center-count",(center.match(/"id":"T\d{4}"/g)||[]).length===1880,"Centre web contient 1880 tests manuels");
need("tests.center-version",center.includes("Centre de tests — V0.9.9")&&center.includes("Régression complète V0.1.x → V0.9.9")&&center.includes("cfg.APP_VERSION==='0.9.9'"),"Centre de tests V0.9.9 cohérent");
...
const stale=[];
for(const rel of ["VERSION","config.js","config.example.js","sw.js","assets/assets-manifest.json","index.html","js/app.js","js/preseason099.js"]){
...
need("version.no-stale-current",stale.length===0,"Aucune référence technique V0.9.8 résiduelle dans les surfaces courantes V0.9.9",stale.join(", "));
...
if(baseUrl){
  ...
  const remoteChecks=[
    ["VERSION",t=>t.trim()==="0.9.9"],
    ["config.js",t=>t.includes('APP_VERSION: "0.9.9"')],
    ["sw.js",t=>t.includes("nid-champions-v0.9.9")&&t.includes("./js/preseason099.js")],
    ["index.html",t=>t.includes("js/preseason099.js")&&t.includes("test-center-v0.9.9.html")&&t.includes("road-check-v0.9.9.html")],
    ...
    ["tests/test-center-v0.9.9.html",t=>t.includes("Centre de tests — V0.9.9")&&t.includes('"id":"T1880"')],
...
const report={version:"0.9.9",generated_at:new Date().toISOString(),base_url:baseUrl,summary,checks};
fs.writeFileSync(path.join(root,"tests","test-report-v0.9.9.json"),JSON.stringify(report,null,2)+"\n");
for(const c of checks)console.log(`${c.status.padEnd(4)} ${c.id} — ${c.message}${c.details?` · ${c.details}`:""}`);
console.log(`\nRésumé V0.9.9: ${summary.passed} PASS · ${summary.warnings} WARN · ${summary.failed} FAIL / ${summary.total}`);
console.log("Rapport: tests/test-report-v0.9.9.json");
