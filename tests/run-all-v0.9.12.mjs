...
need("version.file",read("VERSION").trim()==="0.9.12",`VERSION = ${read("VERSION").trim()||"absent"} (attendu 0.9.12)`);
need("version.config",contains("config.js",'APP_VERSION: "0.9.12"'),"config.js annonce 0.9.12");
need("version.config-example",contains("config.example.js",'APP_VERSION: "0.9.12"'),"config.example.js annonce 0.9.12");
need("version.cache",contains("sw.js","nid-champions-v0.9.12"),"Cache Service Worker V0.9.12");
need("version.assets",contains("assets/assets-manifest.json",'"version": "0.9.12"'),"Manifest des assets V0.9.12");
...
need("v0912r2.mobile-admin-tab",index12.includes('id="mobileDrawerAdminBlockV0912"')&&release12.includes("mobileDrawerAdminBlockV0912"),"Le mobile donne accès à l'Administration via le menu latéral");
need("v0912r2.mobile-layout",css12.includes("menu burger mobile latéral")&&css12.includes(".mobile-drawer-v0912")&&release12.includes("setupMobileDrawerV0912"),"La mise en page mobile utilise le menu latéral");
...
need("v0912r3.no-bottom-nav",!index12.includes("bottom-nav-v0912")&&!index12.includes("mobileMoreNavV0912")&&!index12.includes("mobileAdminNavV0912"),"La barre basse mobile est supprimée");
need("v0912r3.mobile-drawer",index12.includes('id="mobileMenuBtnV0912"')&&index12.includes('id="mobileDrawerV0912"')&&release12.includes("mobileDrawerOpenV0912")&&release12.includes("mobileDrawerCloseV0912"),"Le burger ouvre un panneau latéral gauche");
need("v0912r3.admin-drawer",index12.includes('data-mobile-admin-section="matches"')&&index12.includes('data-mobile-admin-section="players"')&&release12.includes("setAdminSection(section"),"Les rubriques Admin sont intégrées au drawer");
need("v0912r3.admin-nav-hidden",css12.includes("#view-admin .admin-nav{display:none!important}")&&index12.includes('id="adminMobileMenuBtnV0912"'),"La rangée Admin mobile est remplacée par le bouton Rubriques");
...
need("tests.center-current-release",
  center.includes("t=>t.trim()==='0.9.12'") &&
  center.includes("nid-champions-v0.9.12") &&
  center.includes("Cache Service Worker V0.9.12") &&
  center.includes("js/release0912.js"),
  "Centre web contrôle réellement la release courante V0.9.12"
);
...
if(baseUrl){
  ...
  const remoteChecks=[
    ["VERSION",t=>t.trim()==="0.9.12"],
    ["config.js",t=>t.includes('APP_VERSION: "0.9.12"')],
    ["sw.js",t=>t.includes("nid-champions-v0.9.12")&&t.includes("./js/preseason099.js")&&t.includes("./js/preprod0910.js")&&t.includes("./js/release0911.js")&&t.includes("./js/release0912.js")],
...
const report={version:"0.9.12",generated_at:new Date().toISOString(),base_url:baseUrl,summary,checks};
fs.writeFileSync(path.join(root,"tests","test-report-v0.9.12.json"),JSON.stringify(report,null,2)+"\n");
for(const c of checks)console.log(`${c.status.padEnd(4)} ${c.id} — ${c.message}${c.details?` · ${c.details}`:""}`);
console.log(`\nRésumé V0.9.12: ${summary.passed} PASS · ${summary.warnings} WARN · ${summary.failed} FAIL / ${summary.total}`);
console.log("Rapport: tests/test-report-v0.9.12.json");
