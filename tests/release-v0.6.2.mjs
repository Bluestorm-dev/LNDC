...
assert.equal(read('VERSION').trim(),'0.6.2');
assert.match(read('config.js'),/APP_VERSION:\s*"0\.6\.2"/);
assert.match(read('config.example.js'),/APP_VERSION:\s*"0\.6\.2"/);
assert.equal(JSON.parse(read('assets/assets-manifest.json')).version,'0.6.2');
...
const sw=read('sw.js');
assert.match(sw,/nid-champions-v0\.6\.2/);
...
const avatarCss=read('css/avatars.css');assert.match(avatarCss,/un seul rendu joueur/);
...
const context=vm.createContext({
  console,
  window:{
    NIDC_CONFIG:{SUPABASE_URL:'https://YOUR_PROJECT.supabase.co',SUPABASE_ANON_KEY:'YOUR_SUPABASE_ANON_KEY',DEMO_WHEN_UNCONFIGURED:true,APP_VERSION:'0.6.2'},
...
assert.ok(exists('installation/INSTALLATION_V0.6.2.txt'));
assert.ok(exists('docs/TEST_CHECKLIST_V0.6.2.md'));
console.log('V0.6.2 release tests: OK');
