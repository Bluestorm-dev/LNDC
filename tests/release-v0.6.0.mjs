...
assert.equal(read('VERSION').trim(),'0.6.0');
assert.match(read('config.js'),/APP_VERSION:\s*"0\.6\.0"/);
assert.match(read('config.example.js'),/APP_VERSION:\s*"0\.6\.0"/);
assert.equal(JSON.parse(read('assets/assets-manifest.json')).version,'0.6.0');
...
const css=['base.css','predictions.css','ranking.css','clubs.css','champions.css','layout.css','teams.css','avatars.css','communication.css'];
...
const sw=read('sw.js');
assert.match(sw,/nid-champions-v0\.6\.0/);
...
const context=vm.createContext({
  console,
  window:{
    NIDC_CONFIG:{SUPABASE_URL:'https://YOUR_PROJECT.supabase.co',SUPABASE_ANON_KEY:'YOUR_SUPABASE_ANON_KEY',DEMO_WHEN_UNCONFIGURED:true,APP_VERSION:'0.6.0'},
...
console.log('V0.6.0 release tests: OK');
