...
assert.equal(read('VERSION').trim(),'0.5.5');
assert.match(read('config.example.js'),/APP_VERSION:\s*"0\.5\.5"/);
assert.match(read('config.js'),/APP_VERSION:\s*"0\.5\.5"/);
assert.equal(JSON.parse(read('assets/assets-manifest.json')).version,'0.5.5');
...
const sw=read('sw.js');
assert.match(sw,/nid-champions-v0\.5\.5/);
...
assert.ok(exists('installation/INSTALLATION_V0.5.5.txt'));
assert.ok(exists('docs/TEST_CHECKLIST_V0.5.5.md'));
assert.ok(exists('sql/HOTFIX_V0.5.5_EXISTING_DB.sql'));
assert.ok(exists('sql/013_patch_v0.5.5_team_polish.sql'));
assert.match(read('js/teams.js'),/data-team-tab=\"management\"/);
assert.match(read('js/teams.js'),/Transférer puis quitter/);
assert.match(read('js/teams.js'),/avatarCoreHTML\(state\.profile/);
assert.match(read('js/teams.js'),/stripes-diagonal/);
assert.match(read('css/teams.css'),/bg-quarters/);
assert.match(read('index.html'),/id=\"homeHeroAvatar\"/);
assert.match(read('js/predictions.js'),/homeHeroAvatar/);
assert.match(read('sql/HOTFIX_V0.5.5_EXISTING_DB.sql'),/split-vertical/);
assert.match(read('sql/HOTFIX_V0.5.5_EXISTING_DB.sql'),/stripes-diagonal/);
assert.match(read('sql/HOTFIX_V0.5.5_EXISTING_DB.sql'),/quarters/);
...
const smoke=new vm.Script(`
  ...
  ensureDemoTeams();
  ({matches:state.allMatches.length,avatars:OFFICIAL_AVATARS.length,teamShapes:TEAM_SHAPES.length,teamBackgrounds:TEAM_BACKGROUNDS.length,koPhases:KO_PHASES.length,avatarOk:avatarHTML(state.profile).includes('player-avatar-core'),bootOk:typeof boot==='function',adminOk:typeof renderAdmin==='function'});
`).runInContext(context);
assert.deepEqual({...smoke},{matches:8,avatars:90,teamShapes:12,teamBackgrounds:13,koPhases:5,avatarOk:true,bootOk:true,adminOk:true});

console.log('V0.5.5 release tests: OK');
