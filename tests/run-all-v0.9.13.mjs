...
const required=[
  ...
  "css/release0912.css","js/release0912.js",
  "sql/031_patch_v0.9.12_desktop_ux.sql","sql/HOTFIX_V0.9.12_EXISTING_DB.sql","sql/000_INSTALL_FRESH_V0.9.12.sql",
  "tests/test-center-v0.9.12.html","docs/TEST_CHECKLIST_V0.9.12.md","docs/TEST_MATRIX_V0.9.12.md",
  "INSTALLATION_V0.9.12.txt","PATCH_NOTES_V0.9.12.txt","README_TEST_SYSTEM_V0.9.12.md",
  "css/release0913.css","js/release0913.js",
  "sql/032_patch_v0.9.13_mobile_onboarding_accounts.sql","sql/HOTFIX_V0.9.13_EXISTING_DB.sql","sql/000_INSTALL_FRESH_V0.9.13.sql",
  "supabase/functions/admin-delete-user-v0913/index.ts",
  "tests/test-center-v0.9.13.html","docs/TEST_CHECKLIST_V0.9.13.md","docs/TEST_MATRIX_V0.9.13.md",
  "INSTALLATION_V0.9.13.txt","PATCH_NOTES_V0.9.13.txt","README_TEST_SYSTEM_V0.9.13.md"
];
for(const rel of required)need("file:"+rel,exists(rel),exists(rel)?`Présent: ${rel}`:`Absent: ${rel}`);

// Release / cache / intégration.
need("version.file",read("VERSION").trim()==="0.9.13",`VERSION = ${read("VERSION").trim()||"absent"} (attendu 0.9.13)`);
need("version.config",contains("config.js",'APP_VERSION: "0.9.13"'),"config.js annonce 0.9.13");
need("version.config-example",contains("config.example.js",'APP_VERSION: "0.9.13"'),"config.example.js annonce 0.9.13");
need("version.cache",contains("sw.js","nid-champions-v0.9.13"),"Cache Service Worker V0.9.13");
need("version.assets",contains("assets/assets-manifest.json",'"version": "0.9.13"'),"Manifest des assets V0.9.13");
...
need("front.final.links",index.includes('href="tests/test-center-v0.9.12.html"')&&index.includes("1 980 contrôles")&&index.includes('href="tests/test-center-v0.9.11.html"')&&index.includes('href="tests/test-center-v0.9.10.html"')&&index.includes('href="tests/test-center-v0.9.9.html"'),"Admin pointe vers le Centre V0.9.12 et conserve les centres historiques");
...
const release12=read("js/release0912.js"), css12=read("css/release0912.css"), sql12=read("sql/031_patch_v0.9.12_desktop_ux.sql"), index12=read("index.html"), release13=read("js/release0913.js"), css13=read("css/release0913.css"), sql13=read("sql/032_patch_v0.9.13_mobile_onboarding_accounts.sql"), edge13=read("supabase/functions/admin-delete-user-v0913/index.ts"), core12=read("js/core.js"), pred12=read("js/predictions.js"), notif12=read("js/notifications.js");
...
need("v0912r2.mobile-admin-tab",index12.includes('id="adminMobileSectionV0912"')&&release12.includes("syncAdminMobileSectionV0912"),"L'Admin mobile dispose d'un sélecteur compact de rubrique");
need("v0912r2.mobile-layout",css12.includes("la sidebar desktop devient off-canvas")&&release12.includes("setupMobileSidebarV0912"),"La navigation mobile réutilise la sidebar existante");
...
need("v0912r4.no-extra-mobile-menu",!index12.includes("bottom-nav-v0912")&&!index12.includes("mobileDrawerV0912")&&!index12.includes("mobileMoreNavV0912"),"Aucune seconde navigation mobile n'est injectée");
need("v0912r4.logo-opens-sidebar",index12.includes('id="mobileSidebarTriggerV0912"')&&release12.includes("openMobileSidebarV0912")&&css12.includes(".sidebar.mobile-open-v0912"),"Le logo du Nid ouvre la sidebar existante");
...
need("v0912.team-ranking",release12.includes("team-rank-v0912")&&release12.includes("standingForClubV0912"),"Rang C1 injecté dans les cartouches équipes");
need("v0912.club-sheet",release12.includes("openClubSheetV0912")&&release12.includes("data-club-prono-v0912")&&release12.includes("Forme"),"Fiche club avec forme, prochains/derniers matchs et accès au prono");
need("v0912.ucl-tabs",release12.includes('data-ucl-tab="standings"')&&release12.includes('data-ucl-tab="knockout"')&&release12.includes('data-ucl-tab="info"')&&!release12.includes('data-ucl-tab="overview"'),"Centre C1 desktop réduit à Classement / Phases finales / Infos");
need("v0912.ucl-legend",css12.includes("ucl-zones-v0912")&&release12.includes("1–8")&&release12.includes("9–24")&&release12.includes("25–36"),"Légende des zones intégrée au classement C1");
need("v0912.profile-tabs",index12.includes("profileTabsV0912")&&index12.includes("Saison & carrière")&&index12.includes("Préférences"),"Profil desktop organisé en onglets");
need("v0912.avatar-drawer",index12.includes("avatarDrawerV0912")&&index12.includes("toggleAvatarEditorV0912")&&css12.includes("avatar-drawer-v0912.hidden"),"Bibliothèque avatars masquée jusqu'à Changer mon avatar");
need("v0912.ranking-badges-gated",sql12.includes("b.category='classement'")&&sql12.includes("'metric','played'")&&sql12.includes("'value',1"),"Badges classement conditionnés à played >= 1");
need("v0912.bulk-odds-sql",sql12.includes("admin_save_matchday_odds_v0912")&&sql12.includes("admin_update_match_odds_v0910")&&sql12.includes("matchday_odds_bulk_v0912"),"RPC atomique de saisie des cotes d'une journée présente");
...
need("v0912.betclic-r7-preserved",read("supabase/functions/sync-betclic-odds/index.ts").includes('"barcelona":["barcelone"')&&read("supabase/functions/sync-betclic-odds/index.ts").includes('"club brugge":["club bruges"'),"Betclic R7 et alias réels conservés");
need("v0912.badges100",Number(badgeCat.count)===100&&badgeCat.items.length===100,"Les 100 succès restent intégrés");

const matrix=read("docs/TEST_MATRIX_V0.9.12.md");
const ids=[...matrix.matchAll(/\*\*(T\d{4})\*\*/g)].map(m=>m[1]);
need("tests.matrix-count",ids.length===1980,`Matrice: ${ids.length} contrôles (attendu 1980)`);
need("tests.matrix-unique",new Set(ids).size===ids.length,"IDs de la matrice uniques");
need("tests.matrix-last",ids.at(-1)==="T1980",`Dernier contrôle: ${ids.at(-1)||"absent"} (attendu T1980)`);
const center=read("tests/test-center-v0.9.12.html");
need("tests.center-count",(center.match(/"id":"T\d{4}"/g)||[]).length===1980,"Centre web contient 1980 tests manuels");
need("tests.center-version",center.includes("Centre de tests — V0.9.12")&&center.includes("Régression complète V0.1.x → V0.9.12")&&center.includes("cfg.APP_VERSION==='0.9.12'"),"Centre de tests V0.9.12 cohérent");
need("tests.center-diagnostic",center.includes("admin_diagnostics_v0912")&&center.includes("admin_diagnostics_v0911")&&center.includes("admin_diagnostics_v0910")&&center.includes("admin_diagnostics_v099")&&center.includes("v0910.calendar")&&center.includes("v099.runs")&&center.includes("v099.onboarding"),"Centre web exécute le diagnostic V0.9.12 et les diagnostics historiques");
...
const center13=read("tests/test-center-v0.9.13.html");
need("tests.center-current-release",
  center13.includes("V0.9.13") && center13.includes("Mobile, onboarding et comptes"),
  "Centre web V0.9.13 présent pour la release courante"
);
need("tests.center-v099-compat-r2",
  center.includes("x.test==='Version'") &&
  center.includes("app_settings.app_version") &&
  center.includes("backend courant 0.9.12"),
...
need("tests.windows-safe",read("tests/run-all-v0.9.12.mjs").includes("fileURLToPath(import.meta.url)"),"Runner V0.9.12 compatible chemins Windows");
need("docs.checklist-count",(read("docs/TEST_CHECKLIST_V0.9.11.md").match(/\*\*[TX]\d{2,4}\*\*/g)||[]).length===40,"Checklist V0.9.12 contient 40 contrôles spécifiques");
...
if(baseUrl){
  ...
  const remoteChecks=[
    ["VERSION",t=>t.trim()==="0.9.13"],
    ["config.js",t=>t.includes('APP_VERSION: "0.9.13"')],
    ["sw.js",t=>t.includes("nid-champions-v0.9.13")&&t.includes("./js/preseason099.js")&&t.includes("./js/preprod0910.js")&&t.includes("./js/release0911.js")&&t.includes("./js/release0912.js")],
    ["index.html",t=>t.includes("js/preseason099.js")&&t.includes("js/preprod0910.js")&&t.includes("js/release0911.js")&&t.includes("test-center-v0.9.12.html")&&t.includes("js/release0912.js")&&t.includes("road-check-v0.9.9.html")],
    ...
    ["tests/test-center-v0.9.11.html",t=>t.includes("Centre de tests — V0.9.12")&&t.includes('"id":"T1940"')&&t.includes("admin_diagnostics_v0911")],
...
need("v0913.auto-registration",sql13.includes("alter table public.profiles alter column status set default 'active'")&&sql13.includes("values(new.id,v_username,'active')")&&sql13.includes("where status='pending'"),"Les comptes sont validés automatiquement");
need("v0913.new-player-push",sql13.includes('notify_super_admin_registration_v0913')&&sql13.includes("'registration-auto:'")&&sql13.includes('push_requested'),"Le Super Admin est informé des nouvelles inscriptions automatiques");
...
fs.writeFileSync(path.join(root,"tests","test-report-v0.9.13.json"),JSON.stringify(report,null,2)+"\n");
for(const c of checks)console.log(`${c.status.padEnd(4)} ${c.id} — ${c.message}${c.details?` · ${c.details}`:""}`);
console.log(`\nRésumé V0.9.13: ${summary.passed} PASS · ${summary.warnings} WARN · ${summary.failed} FAIL / ${summary.total}`);
console.log("Rapport: tests/test-report-v0.9.13.json");
