...
assert.equal(read('VERSION').trim(),'0.6.4');
assert.match(read('config.js'),/APP_VERSION:\s*"0\.6\.4"/);
assert.match(read('config.example.js'),/APP_VERSION:\s*"0\.6\.4"/);
assert.equal(JSON.parse(read('assets/assets-manifest.json')).version,'0.6.4');
...
const sw=read('sw.js');
assert.match(sw,/nid-champions-v0\.6\.4/);
...
for(const f of ['sql/HOTFIX_V0.6.4_EXISTING_DB.sql','sql/017_patch_v0.6.4_push_immediate_cron_rls.sql','sql/000_INSTALL_FRESH_V0.6.4.sql','sql/ENABLE_PUSH_CRON_V0.6.4_TEMPLATE.sql','installation/INSTALLATION_V0.6.4.txt','docs/TEST_CHECKLIST_V0.6.4.md'])assert.ok(exists(f),`${f} absent`);
...
const context=vm.createContext({
  console,
  window:{
    NIDC_CONFIG:{SUPABASE_URL:'https://YOUR_PROJECT.supabase.co',SUPABASE_ANON_KEY:'YOUR_SUPABASE_ANON_KEY',DEMO_WHEN_UNCONFIGURED:true,APP_VERSION:'0.6.4'},
...
// V0.6.4 : le même joueur doit produire le même composant public, même si la ligne métier contient un ancien avatar.
...
assert.match(notifications,/id="pushSendTestBtn"/);
assert.doesNotMatch(notifications,/pushQuickTestBtn/);
assert.match(notifications,/register_my_push_subscription_v064/);
assert.match(notifications,/admin_schedule_cron_test_v064/);
assert.match(notifications,/cronScheduleTestBtn/);
assert.match(edge,/action==="dispatch-one"/);
assert.match(edge,/bypassQuiet:true/);
assert.match(edge,/cron-test/);
const sql064=read('sql/HOTFIX_V0.6.4_EXISTING_DB.sql');
for(const pattern of [
  /register_my_push_subscription_v064/,
  /dispatch_immediate_push_v064/,
  /admin_schedule_cron_test_v064/,
  /'\* \* \* \* \*'/,
  /values \('app_version','"0\.6\.4"'::jsonb\)/,
  /push_subscriptions_own_select_v064/,
  /push_subscriptions_own_update_v064/
])assert.match(sql064,pattern);
assert.match(read('sql/ENABLE_PUSH_CRON_V0.6.4_TEMPLATE.sql'),/'\* \* \* \* \*'/);
...
assert.ok(exists('installation/INSTALLATION_V0.6.4.txt'));
assert.ok(exists('docs/TEST_CHECKLIST_V0.6.4.md'));
console.log('V0.6.4 release tests: OK');
