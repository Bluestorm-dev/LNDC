...
assert.equal(read('VERSION').trim(),'0.6.0a');
assert.match(read('config.js'),/APP_VERSION:\s*"0\.6\.0a"/);
assert.match(read('config.example.js'),/APP_VERSION:\s*"0\.6\.0a"/);
assert.equal(JSON.parse(read('assets/assets-manifest.json')).version,'0.6.0a');

const js=['core.js','avatars.js','teams.js','notifications.js','rivals.js','support.js','owl.js','auth.js','data.js','champions.js','profile.js','predictions.js','ranking.js','admin.js','realtime.js','app.js'];
const css=['base.css','predictions.css','ranking.css','clubs.css','champions.css','layout.css','teams.css','avatars.css','communication.css','admin.css'];
...
const sw=read('sw.js');
assert.match(sw,/nid-champions-v0\.6\.0a/);
...
for(const pattern of [/defaultNotificationPreferences/,/reminder_3h:true/,/reminder_30m:true/,/reminder_24h:false/,/quiet_start:"23:00"/,/quiet_end:"08:00"/,/Notification\.requestPermission\(\)/,/renderHomePushPrompt/,/sendAdminPushTest/,/sendCriticalSystemMessage/,/category_support/,/if\(n\?\.category===\"system\"\)return true/,/value\.startsWith\(\"matches:\"\)/])assert.match(notifications,pattern);
...
const context=vm.createContext({
  console,
  window:{
    NIDC_CONFIG:{SUPABASE_URL:'https://YOUR_PROJECT.supabase.co',SUPABASE_ANON_KEY:'YOUR_SUPABASE_ANON_KEY',DEMO_WHEN_UNCONFIGURED:true,APP_VERSION:'0.6.0a'},
...
const smoke=new vm.Script(`
  ...
  ({matches:state.allMatches.length,avatars:OFFICIAL_AVATARS.length,teamShapes:TEAM_SHAPES.length,teamBackgrounds:TEAM_BACKGROUNDS.length,notif3h:state.notificationPreferences.reminder_3h,notif30:state.notificationPreferences.reminder_30m,notif24:state.notificationPreferences.reminder_24h,quietStart:state.notificationPreferences.quiet_start,rivalOk:typeof renderRivalView==='function',supportOk:typeof openSupportCenter==='function',owlOk:typeof renderOwlHome==='function',pushTestOk:typeof sendAdminPushTest==='function',profileMessageOk:typeof openAdminOwlMessageForPlayer==='function'});
`).runInContext(context);
assert.deepEqual({...smoke},{matches:8,avatars:90,teamShapes:12,teamBackgrounds:13,notif3h:true,notif30:true,notif24:false,quietStart:'23:00',rivalOk:true,supportOk:true,owlOk:true,pushTestOk:true,profileMessageOk:true});
...
assert.ok(exists('installation/INSTALLATION_V0.6.0a.txt'));
assert.ok(exists('docs/TEST_CHECKLIST_V0.6.0a.md'));
console.log('V0.6.0a release tests: OK');
