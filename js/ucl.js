...
async function loadUclCenterData(force=false){
  ...
  try{
    ...
    if(mr.error||sr.error){
      state.uclCenterError="Centre Ligue des champions non initialisé. Vérifie la configuration dans l’administration.";
...
function renderUclCenter(){
  ...
  root.innerHTML=`<div class="ucl-hero card card-pad"><div><span class="eyebrow gold">Centre Ligue des champions</span><h2>⭐ Toute la C1, sans quitter le Nid</h2><p>Résultats réels, calendrier, classement, phases finales, forme et fiches des clubs.</p></div><button id="uclRefreshBtn" class="btn ${isAdminProfile()?'gold':'secondary'} small">↻ ${isAdminProfile()?'Synchroniser':'Actualiser'}</button></div><div class="ucl-tabs"><button data-ucl-tab="overview" class="${tab==='overview'?'active':''}">Vue d’ensemble</button><button data-ucl-tab="standings" class="${tab==='standings'?'active':''}">Classement</button><button data-ucl-tab="calendar" class="${tab==='calendar'?'active':''}">Calendrier & résultats</button><button data-ucl-tab="knockout" class="${tab==='knockout'?'active':''}">Phases finales</button><button data-ucl-tab="clubs" class="${tab==='clubs'?'active':''}">Clubs</button></div><div id="uclTabBody">${tab==='overview'?uclOverviewHTML():tab==='standings'?uclStandingsHTML():tab==='calendar'?uclCalendarHTML():tab==='knockout'?uclPhaseBracketHTML():uclClubsHTML()}</div>`;
