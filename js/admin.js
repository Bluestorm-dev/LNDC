...
// Le Nid des Champions V0.9.11 — administration modulaire
  ...
  function renderAdminPlayers(){
    ...
    root.innerHTML=(players.length?players.map(p=>`<div class="admin-player-row" data-admin-player="${p.id||p.user_id}">${avatarHTML(p)}<div class="admin-player-copy"><strong>${esc(p.username||"Joueur")}</strong><div class="admin-player-meta"><span class="chip admin-role-${esc(p.role||"player")}">${esc(adminRoleLabel(p.role))}</span><span>${esc(p.status||"active")}</span>${p.first_name?`<span>· ${esc(p.first_name)}</span>`:""}</div></div><div class="admin-player-actions"><button class="btn secondary small" type="button" data-admin-player-open>Voir le profil</button>${state.profile?.role==="super_admin"&&String(p.id||p.user_id)!==String(state.user?.id)&&p.role!=="super_admin"?'<button class="btn danger small" type="button" data-admin-player-delete>Supprimer</button>':''}</div></div>`).join(""):'<div class="empty">Aucun joueur correspondant.</div>')+(all.length>pageSize?`<div class="admin-player-pagination-v095"><button id="adminPlayersPrevV095" class="btn secondary small" ${page===0?'disabled':''}>←</button><span>Page ${page+1} / ${pages}</span><button id="adminPlayersNextV095" class="btn secondary small" ${page>=pages-1?'disabled':''}>→</button></div>`:'');
    $$('[data-admin-player]',root).forEach(row=>{const open=$('[data-admin-player-open]',row);if(open)open.onclick=()=>openPlayerQuickProfile(row.dataset.adminPlayer);const del=$('[data-admin-player-delete]',row);if(del)del.onclick=()=>deletePlayerAccountV0913(row.dataset.adminPlayer);});
