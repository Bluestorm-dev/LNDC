...
// Le Nid des Champions V0.9.5 — chargement des données, saisons et classements serveur
  ...
  async function loadData() {
    ...
    const [{data:matchdays,error:mdErr},{data:clubs,error:clubErr},{data:matches,error:matErr},{data:phases,error:phaseErr},{data:ties,error:tieErr}] = await Promise.all([
      ...
      sb.from("clubs").select("id,name,short_name,tla,country,venue,address,website,founded,club_colors,coach_name,coach_nationality,squad,ucl_titles,ucl_best_result,ucl_history,logo_url,logo_source_url,logo_storage_path,external_provider,external_id,is_active,metadata_source,manual_metadata_lock,manual_metadata_updated_at,provider_metadata_updated_at,provider_details_updated_at").eq("is_active",true).order("name"),
      sb.from("matches").select("id,season_id,phase_id,matchday_id,kickoff_at,stadium,venue_country,status,data_source,is_test,test_enabled,home_score,away_score,points_multiplier,external_provider,external_match_id,external_stage,odds_home,odds_draw,odds_away,odds_provider,odds_bookmaker,odds_source_season,odds_is_test_shifted,odds_updated_at,tie_id,leg_number,went_to_extra_time,penalties_home,penalties_away,winner_club_id,schedule_source,manual_schedule_lock,manual_schedule_updated_at,provider_schedule_updated_at,home_club:clubs!matches_home_club_id_fkey(id,name,short_name,tla,country,venue,logo_url,logo_source_url,logo_storage_path),away_club:clubs!matches_away_club_id_fkey(id,name,short_name,tla,country,venue,logo_url,logo_source_url,logo_storage_path)").eq("season_id",state.season.id).order("kickoff_at"),
