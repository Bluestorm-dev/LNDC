...
// Le Nid des Champions V0.9.5 — chargement des données, saisons et classements serveur
  ...
  async function loadData() {
    ...
    const [{data:matchdays,error:mdErr},{data:clubs,error:clubErr},{data:matches,error:matErr},{data:phases,error:phaseErr},{data:ties,error:tieErr}] = await Promise.all([
      ...
      sb.from("clubs").select("id,name,short_name,tla,country,venue,logo_url,logo_source_url,logo_storage_path,external_provider,external_id,is_active,metadata_source,manual_metadata_lock,manual_metadata_updated_at,provider_metadata_updated_at").eq("is_active",true).order("name"),
