...
(() => {
  ...
  function openClubSheetV101(clubId){
    ...
    syncUclLiveFromMatchesV101();
    const standing=safe(state.uclStandings).find(r=>String(r.club_id)===String(clubId));const matches=safe(state.uclMatches).filter(m=>String(m.home_club?.id)===String(clubId)||String(m.away_club?.id)===String(clubId)).sort((a,b)=>new Date(a.kickoff_at)-new Date(b.kickoff_at));
    ...
    const hist=clubHistoryV101(clubId);
    const root=modal(club.name,`<div class="club-sheet-v101">
      <header class="club-hero-v101">${crestHTML(club,true)}<div><span class="eyebrow gold">Fiche club · Ligue des champions</span><h2>${esc(club.name)}</h2><p>${esc([club.short_name&&club.short_name!==club.name?club.short_name:null,club.tla,club.country].filter(Boolean).join(" · "))}</p></div>${state.uclLiveProvisionalV101?'<span class="club-live-v101">● LIVE</span>':""}</header>
      <div class="club-facts-v101"><span><small>Pays</small><b>${esc(club.country||"—")}</b></span><span><small>Stade</small><b>${esc(club.venue||"—")}</b></span><span><small>Fondé</small><b>${club.founded?Number(club.founded):"—"}</b></span><span><small>Entraîneur</small><b>${esc(club.coach_name||"—")}</b></span><span><small>Nom court</small><b>${esc(club.short_name||club.tla||"—")}</b></span><span><small>Couleurs</small><b>${esc(club.club_colors||"—")}</b></span></div>
      ${club.address?`<p class="club-address-v101">📍 ${esc(club.address)}</p>`:""}${club.website?`<p class="club-address-v101">🌐 ${esc(club.website)}</p>`:""}
      <section class="club-c1-summary-v101"><div><span>Classement C1</span><strong>${standing?`#${Number(standing.position)}`:"—"}</strong></div><div><span>Points</span><strong>${Number(standing?.points||0)}</strong></div><div><span>V-N-D</span><strong>${standing?`${Number(standing.won||0)}-${Number(standing.draw||0)}-${Number(standing.lost||0)}`:"—"}</strong></div><div><span>Diff.</span><strong>${standing?`${Number(standing.goal_difference||0)>0?"+":""}${Number(standing.goal_difference||0)}`:"—"}</strong></div></section>
      <div class="club-feature-grid-v101"><article><span class="eyebrow">Meilleur buteur C1</span><h3>${esc(best?.player_name||"—")}</h3><p>${best?`${Number(best.goals||0)} but${Number(best.goals||0)>1?"s":""} · ${Number(best.assists||0)} passe(s) décisive(s)`:"Donnée en attente de synchronisation."}</p></article><article><span class="eyebrow">Cartons C1</span><h3>${totals.y} 🟨 · ${totals.yr} 🟨🟥 · ${totals.r} 🟥</h3><p>${cards.length?`${cards.length} joueur(s) sanctionné(s)`:'Aucun carton enregistré.'}</p></article><article><span class="eyebrow">Historique C1 dans le Nid</span><h3>${hist.played} match${hist.played>1?"s":""}</h3><p>${hist.w} V · ${hist.d} N · ${hist.l} D · ${hist.gf}-${hist.ga} buts</p></article></div>
      <section><div class="section-title compact"><div><span class="eyebrow gold">Effectif</span><h3>Composition & joueurs</h3></div><span class="chip">${squad.length||"—"}</span></div>${groups.length?`<div class="club-squad-v101">${groups.map(([label,items])=>`<div><h4>${label}</h4>${items.map(p=>`<span><b>${esc(p.name)}</b><small>${esc(p.position||p.nationality||"")}</small></span>`).join("")}</div>`).join("")}</div>`:'<div class="empty">L’effectif sera chargé lors de la prochaine synchronisation C1.</div>'}</section>
      <section><div class="section-title compact"><div><h3>Prochains matchs</h3><p>Un clic ouvre la journée de pronostics.</p></div></div><div class="club-fixtures-grid-v101">${next.length?next.map(m=>clubFixtureMiniV101(m,clubId)).join(""):'<div class="empty">Aucun match à venir.</div>'}</div></section>
      <section><div class="section-title compact"><div><h3>Derniers résultats</h3></div></div><div class="club-fixtures-grid-v101">${recent.length?recent.map(m=>clubFixtureMiniV101(m,clubId)).join(""):'<div class="empty">Aucun résultat.</div>'}</div></section>
    </div>`);
    root.querySelector(".modal-card")?.classList.add("club-sheet-modal-v101");
