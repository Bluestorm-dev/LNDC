...
// Le Nid des Champions V0.5.5a — avatars : catalogue et rendu
  ...
  function avatarCoreHTML(profile,{allowPending=false}={}) {
    ...
    return `<span class="avatar player-avatar-core ${pending&&allowPending?'avatar-pending':''}"><img src="${esc(url)}" alt="Avatar de ${esc(p?.username||'joueur')}" loading="lazy" onerror="this.remove();this.parentElement.classList.add('avatar-broken')"><b aria-hidden="true">${esc(initial)}</b>${pending&&allowPending?'<em title="En attente de modération">⏳</em>':''}</span>`;
  }
  function avatarHTML(profile,opts={}) {
    ...
    if(!team) return core;
    return `<span class="team-avatar ${teamClass(team)}" style="${teamVisualVars(team)}" title="${esc(team.team_name||team.name||'Team')}">${core}<i>${teamLogoHTML(team)}</i></span>`;
  }
