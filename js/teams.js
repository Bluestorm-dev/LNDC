...
// Le Nid des Champions V0.5.4 — Teams
  ...
  function refreshDemoTeamState() {
    ...
    state.teamDirectory=activeTeams.map(t=>({...t,team_id:t.id,captain_username:state.demoUsers.find(u=>u.id===t.captain_user_id)?.username||"—",member_count:memberships.filter(m=>m.team_id===t.id&&!m.left_at).length}));
  ...
  function teamEventHTML(e){
    const labels={team_created:"Team créée",member_joined:"Nouveau membre",member_left:"Départ",member_kicked:"Membre exclu",captain_transferred:"Capitanat transféré",identity_changed:"Identité modifiée",team_dissolved:"Team dissoute",join_requested:"Demande d'adhésion",join_rejected:"Demande refusée",invite_regenerated:"Code d'invitation renouvelé"};
  ...
  function renderTeamDirectory(){
    ...
    root.innerHTML=teams.length?teams.map(t=>{
      const mine=String(t.team_id)===String(state.myTeam?.team_id),fav=teamFavoriteClub(t);
      return `<article class="card team-directory-card" style="${teamVisualVars(t)}"><div class="team-directory-identity">${teamBadgeHTML(t,true)}<div><span class="eyebrow">${t.visibility==='private'?'🔒 Team privée':'🌐 Team publique'}</span><h3>${esc(t.name)}</h3>${t.slogan?`<p>« ${esc(t.slogan)} »</p>`:''}</div></div><div class="team-directory-meta"><span>👑 ${esc(t.captain_username||'—')}</span><span>${Number(t.member_count||0)} membre${Number(t.member_count||0)>1?'s':''}</span>${fav?`<span>${crestHTML(fav)} ${esc(fav.short_name||fav.name)}</span>`:''}</div><div class="team-directory-actions">${mine?'<span class="chip">MA TEAM</span>':state.myTeam?'<span class="muted">Une seule Team active</span>':t.visibility==='public'?`<button class="btn small" data-join-public="${t.team_id}">Rejoindre</button>`:`<button class="btn secondary small" data-request-private="${t.team_id}">Demander à rejoindre</button>`}<button class="text-action" data-view-team="${t.team_id}">Voir</button></div></article>`;
    }).join(''):'<div class="empty">Aucune Team ne correspond à la recherche.</div>';
    $$('[data-join-public]',root).forEach(b=>b.onclick=()=>joinPublicTeam(b.dataset.joinPublic));$$('[data-request-private]',root).forEach(b=>b.onclick=()=>requestPrivateTeam(b.dataset.requestPrivate));$$('[data-view-team]',root).forEach(b=>b.onclick=()=>showPublicTeam(b.dataset.viewTeam));
  }
  ...
  async function dissolveMyTeam(){if(!confirm('Dissoudre définitivement cette Team ? Son historique restera archivé.'))return;try{if(demoMode){const {teams,memberships}=ensureDemoTeams();const t=teams.find(x=>x.id===state.myTeam.id);t.status='dissolved';t.dissolved_at=new Date().toISOString();memberships.filter(m=>m.team_id===t.id&&!m.left_at).forEach(m=>{m.left_at=t.dissolved_at;m.leave_type='dissolved';});localStorage.setItem('nidc_demo_teams',JSON.stringify(teams));localStorage.setItem('nidc_demo_team_memberships',JSON.stringify(memberships));demoLogTeam(t.id,'team_dissolved',state.user.id,null,{name:t.name});}else{const {error}=await sb.rpc('dissolve_team_v050',{p_team_id:state.myTeam.team_id});if(error)throw error;}await reloadTeamsAfterMutation();toast('Team dissoute. Son histoire reste dans les archives.');}catch(err){toast(friendlyError(err),'error');}}

  function renderAdminTeams(){const root=$("#adminTeamsPanel");if(!root)return;const q=String($("#adminTeamSearch")?.value||'').toLocaleLowerCase('fr');const rows=(state.teamDirectory||[]).filter(t=>!q||`${t.name} ${t.captain_username}`.toLocaleLowerCase('fr').includes(q));root.innerHTML=rows.length?rows.map(t=>`<div class="admin-team-row">${teamBadgeHTML(t)}<div><strong>${esc(t.name)}</strong><small>👑 ${esc(t.captain_username||'—')} · ${Number(t.member_count||0)} membre${Number(t.member_count||0)>1?'s':''} · ${t.status}</small></div><button class="btn secondary small" data-admin-open-team="${t.team_id}">Voir</button></div>`).join(''):'<div class="empty">Aucune Team.</div>';$$('[data-admin-open-team]',root).forEach(b=>b.onclick=()=>adminOpenTeam(b.dataset.adminOpenTeam));}
  async function adminOpenTeam(teamId){try{let members=[],events=[];if(demoMode){const old=state.myTeam,stateMy=state.myTeam;const t=state.teamDirectory.find(x=>x.team_id===teamId);members=ensureDemoTeams().memberships.filter(m=>m.team_id===teamId&&!m.left_at).map(m=>{const p=state.demoUsers.find(u=>u.id===m.user_id)||{};return{user_id:m.user_id,username:p.username,is_captain:t?.captain_user_id===m.user_id};});events=JSON.parse(localStorage.getItem('nidc_demo_team_events')||'[]').filter(e=>e.team_id===teamId);}else{const [m,e]=await Promise.all([sb.rpc('get_team_members_v050',{p_team_id:teamId}),sb.rpc('get_team_history_v050',{p_team_id:teamId})]);if(m.error)throw m.error;if(e.error)throw e.error;members=m.data||[];events=e.data||[];}const t=state.teamDirectory.find(x=>String(x.team_id)===String(teamId));const root=modal(`Admin · ${t?.name||'Team'}`,`<div class="admin-team-modal">${t?`<div class="public-team-head">${teamBadgeHTML(t,true)}<div><h2>${esc(t.name)}</h2><p>👑 ${esc(t.captain_username||'—')} · ${t.visibility}</p></div></div>`:''}<h4>Membres</h4>${members.map(m=>`<div class="admin-team-member"><span>${m.is_captain?'👑 ':''}${esc(m.username)}</span>${!m.is_captain?`<button class="text-action" data-admin-transfer="${m.user_id}">Nommer capitaine</button>`:''}</div>`).join('')}<h4>Derniers événements</h4>${events.slice(0,10).map(teamEventHTML).join('')||'<div class="empty">Aucun événement.</div>'}<div class="actions"><button class="btn danger small" id="adminDissolveTeam">Dissoudre la Team</button></div></div>`);$$('[data-admin-transfer]',root).forEach(b=>b.onclick=async()=>{try{if(demoMode){const {teams}=ensureDemoTeams();teams.find(x=>x.id===teamId).captain_user_id=b.dataset.adminTransfer;localStorage.setItem('nidc_demo_teams',JSON.stringify(teams));}else{const {error}=await sb.rpc('transfer_team_captain_v050',{p_team_id:teamId,p_new_captain_user_id:b.dataset.adminTransfer});if(error)throw error;}$("#modalRoot").innerHTML='';await reloadTeamsAfterMutation();toast('Capitaine corrigé.');}catch(err){toast(friendlyError(err),'error');}});$("#adminDissolveTeam",root).onclick=async()=>{if(!confirm('Dissoudre cette Team depuis l’administration ?'))return;try{if(demoMode){const {teams}=ensureDemoTeams();teams.find(x=>x.id===teamId).status='dissolved';localStorage.setItem('nidc_demo_teams',JSON.stringify(teams));}else{const {error}=await sb.rpc('dissolve_team_v050',{p_team_id:teamId});if(error)throw error;}$("#modalRoot").innerHTML='';await reloadTeamsAfterMutation();toast('Team dissoute par Admin.');}catch(err){toast(friendlyError(err),'error');}};}catch(err){toast(friendlyError(err),'error');}}
