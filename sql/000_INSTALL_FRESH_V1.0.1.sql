-- Le Nid des Champions — INSTALLATION FRAÎCHE V0.9.12
-- Base complète jusqu'à V0.9.12.

-- Le Nid des Champions — INSTALLATION FRAÎCHE V0.9.11
-- Base complète jusqu'à V0.9.11.
-- Assemblage : V0.9.10 + R3 calendrier/cotes + V0.9.11 cumulatif.
-- À exécuter uniquement sur une base neuve.

-- LE NID DES CHAMPIONS — INSTALLATION FRAICHE V0.9.0
-- Généré depuis V0.8.0 + diagnostic V0.8.1 + migration V0.9.0.
...
create table if not exists public.profiles (
  ...
  avatar_key text not null default 'avatar-hibou-humour-personnages-chanceux',
  club_heart text,
...
update public.profiles set avatar_key='avatar-hibou-humour-personnages-chanceux' where avatar_key='owl-gold';
update public.profiles set avatar_key='avatar-hibou-humour-personnages-chanceux' where avatar_key='owl-blue';
update public.profiles set avatar_key='avatar-hibou-humour-personnages-chanceux' where avatar_key='owl-violet';
update public.profiles set avatar_key='avatar-hibou-humour-personnages-chanceux' where avatar_key is null or btrim(avatar_key)='';
...
-- -----------------------------------------------------------------------------
-- Catalogue officiel courant : 168 avatars réellement illustrés (clubs + humour).
-- -----------------------------------------------------------------------------
...
-- V0.7.1 correctif complémentaire
...
-- ============================================================================
-- PATCH V0.7.3 — rôles Team indépendants
-- ============================================================================
...
with seasons_scope as (
  ...
  select season_id,season_name,'podium'::text category,rank::integer as "position",user_id,username,points value,
    case rank when 1 then 'Champion' when 2 then 'Vice-champion' else 'Troisième' end label,'{}'::jsonb metadata
  from lb where rank<=3
), best_score as (
  select season_id,season_name,'best_score'::text category,1::integer as "position",user_id,username,points::numeric value,'Meilleur scoreur'::text label,
  ...
  select season_id,season_name,'best_exact'::text category,1::integer as "position",user_id,username,exact_scores::numeric value,'Maître des scores exacts'::text label,'{}'::jsonb metadata
  ...
  select s.id season_id,s.name season_name,'poele_or'::text category,1::integer as "position",e.user_id,p.username,sum(e.points)::numeric value,'Poêle d''Or'::text label,jsonb_build_object('events',count(*)) metadata
  ...
  select s.id season_id,s.name season_name,'genius'::text category,1::integer as "position",e.user_id,p.username,sum(e.points)::numeric value,'Génie de la saison'::text label,jsonb_build_object('events',count(*)) metadata
  ...
  select s.id season_id,s.name season_name,'solitary'::text category,1::integer as "position",h.user_id,h.username,h.solitary_points::numeric value,'Hibou solitaire'::text label,jsonb_build_object('successes',h.successes,'unique_successes',h.unique_successes) metadata
  from seasons_scope s cross join lateral public.get_hibou_solitaire_leaderboard_v080(s.id) h where h.rank=1
), teams as (
  select s.id season_id,s.name season_name,'team'::text category,1::integer as "position",null::uuid user_id,t.team_name username,t.average_points::numeric value,'Meilleure Team'::text label,jsonb_build_object('team_id',t.team_id,'top3_points',t.top3_points) metadata
  ...
  select s.id season_id,s.name season_name,'record'::text category,
    row_number() over(partition by s.id order by r.achieved_at desc,r.record_key)::integer as "position",
...
-- Auto-validation comptes + onboarding V0.9.14
begin;
...
select * from (values
 ('Version','Backend',case when coalesce((select value#>>'{}' from public.app_settings where key='app_version'),'')='0.9.14' then 'PASS' else 'FAIL' end),
...
-- =============================================================================
-- V1.0.1 incremental additions (also valid on a fresh schema)
-- =============================================================================
